...
- **Button toggle (selection) state — NOT IMPLEMENTED (open feature).** The M3 buttons color spec defines full `Toggle unselected` and `Toggle selected` color mappings per variant (e.g. filled selected = Primary/On-primary; filled unselected = Surface-container/On-surface-variant; outlined selected = Inverse-surface/Inverse-on-surface), and the config matrix lists Toggle as an Expressive feature. `Button` has no toggle/selected API. This is a net-new capability (like IconButton's existing `toggle` prop) rather than a token fix — deferred as a feature addition, not counted as compliant.
