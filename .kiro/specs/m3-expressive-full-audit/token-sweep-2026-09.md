...
### Deferred — needs product/spec sign-off (do NOT change blindly)
- **A2 / B1 — Button & family height scale.** MDC `CommonButton.md` confirms the 5-size Expressive scale (xs/s/m/l/xl), round+square shapes, shape-morph, and small-button padding (16dp recommended / 24dp deprecated), but publishes the exact M/L/XL container heights only as images, not dp values. M3 Expressive is widely cited as XS 32 / S 40 / M 56 / L 96 / XL 136dp. This library ships XS 32 / S 40 / M 48 / L 56 / XL 64 (matches XS/S). Raising M/L/XL to the tall Expressive values is a **high-blast-radius visual change** affecting every consumer using `size="m/l/xl"`, and would also require reconciling the SplitButton/ButtonGroup scale (xs32/s36/m40/l48/xl56). **Decision required** before touching. Also standardize the family so a given size name means one height everywhere.
