  ...
  ({ className, icon = "close", onPress, ...props }, ref) => (
    <div ref={ref} className={cn("shrink-0", className)} {...props}>
      <button
        type="button"
        onClick={onPress}
        className="flex items-center justify-center w-12 h-12 rounded-full hover:bg-surface-foreground/[0.08] focus-visible:bg-surface-foreground/[0.10] active:bg-surface-foreground/[0.10] transition-colors"
...
const ToolbarAction = React.forwardRef<HTMLButtonElement, ToolbarActionProps>(
  ({ className, icon, label, ...props }, ref) => (
    <button
      ...
      aria-label={label}
      className={cn(
        "flex items-center justify-center w-12 h-12 rounded-full hover:bg-surface-foreground/[0.08] focus-visible:bg-surface-foreground/[0.10] active:bg-surface-foreground/[0.10] transition-colors",
        className
      )}
      {...props}
    >
